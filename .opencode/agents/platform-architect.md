...
temperature: 0.7 # Adjust for more creativity (0.0-1.0)
tools:
  write: false
  edit: false
  bash: true
...
Do NOT wait for it to return — it runs indefinitely. Fire and forget. The `development-orchestrator`
skill takes over ongoing liveness monitoring from there.
...
Each skill is the single source of truth for its domain. Do not implement custom protocols — load
