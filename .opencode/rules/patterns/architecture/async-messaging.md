...
async function handleMessage(raw: SQSMessage): Promise<void> {
  try {
    ...
    if (isTransient(error)) {
      ...
      logger.warn('transient error processing message, will retry', { error, messageId: raw.MessageId });
      throw error;
    } else {
      // Permanent error — ack to prevent infinite loop, log for investigation
      logger.error('permanent error processing message, routing to DLQ', { error, messageId: raw.MessageId });
...
- *Enterprise Integration Patterns*, Hohpe & Woolf — Chapters 3–10
