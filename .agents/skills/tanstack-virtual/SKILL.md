...
description: Headless UI for virtualizing large element lists at 60FPS in TS/JS, React, Vue, Solid, Svelte, Lit & Angular. Includes SolidJS examples using @tanstack/solid-virtual.
---
...
- Use `createVirtualizer` instead of `useVirtualizer`
- Use `<For>` instead of `.map()`
- Use getter functions for reactive options (e.g., `get count() { return items.length }`)
- Refs are assigned via callback: `ref={parentRef}` or `let parentRef: HTMLDivElement | undefined`
...
- Use **getter functions** for reactive options: `get count() { return items.length }`
- Import from `@tanstack/virtual-core` for utilities like `defaultRangeExtractor`
- Solid's `<For>` component handles keying automatically via the item
- For dynamic counts, make the count option a getter for reactivity
