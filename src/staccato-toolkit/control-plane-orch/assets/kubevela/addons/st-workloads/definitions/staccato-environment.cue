...
template: {
	...
	// exists before the Job mounts it.
	output: {
	...
	parameter: {
		...
		//+usage=Auth provider configuration
		"auth-provider": *{enabled: false} | {...}

		//+usage=Observability provider configuration
		"observability-provider": *{enabled: false} | {...}

		//+usage=Dev tooling and ops service toggles (headlamp, penpot, dev-portal,
		//+usage=external-secrets, wasmcloud). Passed through for downstream consumers;
		//+usage=they do not contribute chart entries to the kustomization.yaml.
