...
`staccato-environment.cue`. The staccato-environment OAM component emits a ConfigMap whose
...
### Requirement: staccato-environment emits kustomization.yaml in ConfigMap data

The `staccato-environment` CUE component definition SHALL accept a `charts` parameter (list of Helm
...
- **WHEN** an `staccato-environment` component is rendered with one or more `charts` entries
