---
td-board: adopt-grafana-alloy-backstage-observability-alloy-deployment
td-issue: td-fa5ed1
---
...
Defines the requirements for deploying Grafana Alloy as a DaemonSet in the `monitoring` namespace, configured with an `config.alloy` pipeline that collects Kubernetes pod logs and receives OTLP signals from Backstage, routing all telemetry to the appropriate Grafana LGTM backends.
